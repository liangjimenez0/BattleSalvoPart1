...
public class ControllerImp implements Controller {
  ...
  public ControllerImp(Readable input, Appendable output, Board playerBoard,
    ...
    this.playerBoard = playerBoard;
    this.opponentBoard = opponentBoard;
    this.userPlayer = new ManualPlayer(playerBoard, opponentBoard, new Random(1));
  ...
  public void run() throws IOException {
    ...
    while (gameOver.isGameOver(userShips, aiShips, playerBoard, opponentBoard) == null) {
      makeShots(opponentBoard, playerBoard, userShips, boardWidth, boardHeight);
    }

    viewer.endPage(gameOver.isGameOver(userShips, aiShips, playerBoard, opponentBoard));
  }
  ...
  private void makeShots(Board opponentBoard, Board playerBoard, List<Ship> userShips,
    ...
    int shipsLeft = Math.min(gameOver.shipsNotSunk(userShips, playerBoard).size(),
        playerBoard.countEmptyCoordinates());
    ...
    while (!validShots(boardWidth, boardHeight, allCoords, userShips, shipsLeft)) {
   ...
   * @param listOfShips a list of Ship on the player's board
   * @return whether the shots inputted by the user are valid
   * @throws IOException throws an IO exception if an error occurs
   */
  private boolean validShots(int boardWidth,
                             int boardHeight, List<Coord> coord, List<Ship> listOfShips,
                             int shipsLeft)
      throws IOException {
